		...
		<>
			{id === 'state' && (
				<div>
					...
					<Select menuPortalTarget={document.body} aria-labelledby={id} id={id} options={states} defaultValue={state} onChange={setState} />
				</div>
			)}
			{id === 'department' && (
				<div>
					<label htmlFor={id}>Department</label>
					<Select menuPortalTarget={document.body} aria-labelledby={id} id={id} options={departments} defaultValue={department} onChange={setDepartment} />
				</div>
			)}
		</>
	);
